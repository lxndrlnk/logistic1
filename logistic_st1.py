...
def main():
    ...
    with st.sidebar:
        st.title("Параметры оптимизации")
         ...
         # Загрузка файлов
        uploaded_car_file = st.sidebar.file_uploader("Загрузите файл car.xlsx", type=['xlsx'])
        uploaded_delivery_file = st.sidebar.file_uploader("Загрузите файл delivery.xlsx", type=['xlsx'])
        ...
        if submitted:    
            with header_container:
                if uploaded_car_file and uploaded_delivery_file:
                    # Обработка файлов
                    try:
                        df_trucks = pd.read_excel(uploaded_car_file)
                        truck_names = ['truck', 'truck_weight', 'truck_volume', 'truck_palletes']
                        df_trucks.columns = truck_names
                        st.success(f"Файл '{uploaded_car_file.name}' успешно прочитан и обработан.")
                    except Exception as e:
                        st.error(f"Ошибка при чтении файла '{uploaded_car_file.name}': {str(e)}")

                    try:
                        df_route = pd.read_excel(uploaded_delivery_file)
                        df_route = df_route.dropna(subset=['ДОЛГОТА', 'ШИРОТА'])
                        ride_names = ['document', 'doc_weight', 'doc_vol', 'doc_palletes', 'lon', 'lat']
                        df_route.columns = ride_names
                        st.success(f"Файл '{uploaded_delivery_file.name}' успешно прочитан и обработан.")
                    except Exception as e:
                        st.error(f"Ошибка при чтении файла '{uploaded_delivery_file.name}': {str(e)}")
                else:
                    if not uploaded_car_file:
                        st.error("Файл 'car.xlsx' не найден.")
                    if not uploaded_delivery_file:
                        st.error("Файл 'delivery.xlsx' не найден.")
                ...
                def create_delivery_points_xlsx():
                    columns = [
                ...
                excel_file = create_delivery_points_xlsx()
                ...
                with zipfile.ZipFile(zip_buffer, 'a', zipfile.ZIP_DEFLATED, False) as zip_file:
                    ...
                    zip_file.writestr('Загрузка машин и порядок доставки.xlsx', excel_file.getvalue())
                ...
                zip_buffer.seek(0)
                st.download_button('Скачать файлы: Загрузка машин и порядок доставки.xlsx, Кластеры и домашний регион.html и Карта маршрутов.html одним файлом (ZIP). Страница будет перезагружена и очищена.', zip_buffer, 'all_files.zip', 'application/zip')
